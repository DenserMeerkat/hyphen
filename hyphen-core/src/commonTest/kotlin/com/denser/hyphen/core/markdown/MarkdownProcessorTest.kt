...
class MarkdownProcessorTest {
    ...
    fun `process correctly strips inline markdown and generates accurate spans`() {
        ...
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 0))
    ...
    fun `process handles multiple inline styles sequentially`() {
        ...
        val boldSpan = assertNotNull(result.newSpans.find { it.style == MarkupStyle.Bold })
        assertEquals(0, boldSpan.start)
        assertEquals(4, boldSpan.end) // "Bold"

        val italicSpan = assertNotNull(result.newSpans.find { it.style == MarkupStyle.Italic })
    ...
    fun `process preserves block prefixes in text but still applies spans`() {
        ...
        for ((text, expectedStyle) in textsAndStyles) {
            val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 0), "Failed on: $text")

            // Block styles should NOT strip the prefix from the cleanText
            assertEquals(text, result.cleanText)

            val span = assertNotNull(result.newSpans.find { it.style == expectedStyle })
    ...
    fun `process identifies checkbox block styles`() {
        ...
        for ((text, expectedStyle) in textsAndStyles) {
            ...
            val span = assertNotNull(
                result.newSpans.find { it.style == expectedStyle },
                "Missing expected $expectedStyle for text: $text"
            )
            assertEquals(0, span.start)
            assertEquals(text.length, span.end)
        }
    ...
    fun `cursor shifts correctly when typing AFTER an inline markdown block`() {
        // Raw text: "**abc**d"
        // Cursor is at the very end (index 8)
        val text = "**abc**d"
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 8))
    ...
    fun `cursor shifts correctly when typing INSIDE an inline markdown block`() {
        ...
        val text = "**ab**"
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 3))
    ...
    @Test
    fun `cursor snaps safely if caught INSIDE a deleted suffix markdown tag`() {
        // Raw text: "**abc**"
        // Cursor is right in the middle of the ending tag (index 6, between the * and *)
        val text = "**abc**"
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 6))
    ...
    @Test
    fun `cursor snaps safely if caught INSIDE a deleted prefix markdown tag`() {
        // Raw text: "**abc**"
        // Cursor is right in the middle of the starting tag (index 1, between the * and *)
        val text = "**abc**"
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 1))

        // Cursor should snap safely to the start of the inner text
        assertEquals(0, result.newCursorPosition)
    }

    @Test
    fun `process detects explicitly closed styles when cursor is at the end of the tag`() {
        val text = "**abc**"
        // Cursor is EXACTLY at the end of the typed tag (index 7)
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 7))

        assertEquals(setOf(MarkupStyle.Bold), result.explicitlyClosedStyles)
    }

    @Test
    fun `process respects inlineOnly flag to ignore block styles`() {
        val text = "- [ ] Task and **Bold**"
        val result = assertNotNull(MarkdownProcessor.process(text, cursorPosition = 0, inlineOnly = true))

        // It should strip the bold syntax, but completely ignore the checkbox
        assertEquals("- [ ] Task and Bold", result.cleanText)

        // Only Bold span should be present
        assertEquals(1, result.newSpans.size)
        assertEquals(MarkupStyle.Bold, result.newSpans.first().style)
    }
}